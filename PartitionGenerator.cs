    ...
    public class PartitionGenerator
    {
        ...
        public async Task<List<Partition>> GeneratePartitions()
        {
            ...
            while (dataToPartition.TryPop(out Partition nextPartition))
            {
                ...
                {
                    Console.WriteLine($"PARTITION ADDED: Count: {nextPartition.DocumentCount} LowerBound: {nextPartition.LowerBound} UpperBound: {nextPartition.UpperBound} Index: {index} PartitionCount: {partitions.Count}\n");
                ...
                Console.WriteLine($"CurrenCount: {nextPartition.DocumentCount} LowerBound: {nextPartition.LowerBound} UpperBound: {nextPartition.UpperBound} Midpoint: {midpoint} Index: {index} PartitionCount: {partitions.Count}\n");
        ...
        {
            IEnumerator<Partition> partitionEnumerator = partitions.OrderBy(x => x.LowerBound).GetEnumerator();
            ...
            {
                if (nextPartition?.DocumentCount != 0) mergedPartitions.Add(nextPartition);
